...
`AdsExecuteSQLDirect` / `AdsExecuteSQL` deixavam o cursor do
resultado *antes* da primeira linha, onde o SAP ADS o posiciona
*sobre* a primeira linha — ler o registro atual logo após executar
retornava uma linha vazia fantasma em cada consulta. O resultado
agora cai na linha 1, igual ao SAP, para tabelas do motor, resultados
`system.*`/agregados, cursores de backends SQL e clientes remotos
igualmente. Chamadores que já fazem `AdsGotoTop` primeiro não são
afetados.

### REMOTO — read-ahead para trás (PgUp)

O read-ahead era só para frente: um browse com `Skip(-1)` pagava uma
viagem de rede por linha. Percursos reversos agora também fazem
prefetch — um percurso reverso de 300 linhas caiu de 299 requisições
de rede para 7 — e um novo bit de capacidade mantém corretas as
combinações de cliente/servidor antigos e novos em ambas as direções.

### A versão real do servidor agora é visível a partir do cliente

Relatos repetidos de "a correção não funcionou" continuavam se
resolvendo em um `openads_serverd` antigo ainda em execução (um
serviço do Windows ativo mantém seu exe aberto, então copiar uma
atualização por cima pode falhar silenciosamente) — e não havia como
provar isso do lado do cliente: o handshake do protocolo e
`AdsMgGetInstallInfo` respondiam strings fixas. Agora o handshake
carrega a versão real de compilação (`openads/1.8.14`), e
`AdsMgGetInstallInfo` em um handle de gerenciamento **remoto**
informa a versão do **servidor** — contra qualquer servidor já
publicado (builds antigos se identificam como `0.3.2`). No Harbour:
`AdsMgConnect("host:port")`, `AdsMgGetInstallInfo()[3]`,
`AdsMgDisconnect()`.

### REMOTO — `CloseTable` mantinha os arquivos da tabela abertos no servidor

Fechar uma tabela remota liberava a visão do cliente, mas o handle
"sombra" do lado servidor (usado para navegação ordenada, scopes e
travas) permanecia aberto até a sessão desconectar — então apagar,
renomear ou reabrir em exclusivo o `.dbf`/`.cdx`/`.fpt` recém-fechado
falhava com "arquivo em uso", e os loops de retry da aplicação
transformavam isso em segundos de espera ou um aparente travamento na
hora de fechar todos os arquivos. `CloseTable` agora fecha o handle
sombra junto com a tabela.

Atualize `openads_serverd` para a correção do CloseTable; atualize
também `openace64.dll` para ver a versão do servidor a partir do
cliente. Novo exemplo: `examples/fivewin/xbpaint_delscope.prg`
(registra a versão do servidor e reproduz a forma exata de repintura
do xBrowse — leituras de página, `AdsGetRelKeyPos`/`AdsSetRelKeyPos`,
arrastes da barra de rolagem — sobre uma tabela remota com scope e
`SET DELETED ON`).
...
### REMOTO — browses com ordem/scope: correções + read-ahead

Conclui o trabalho remoto de `SET DELETED`: alterar `AdsShowDeleted`
agora invalida o bloco de leitura antecipada do cliente (registros
apagados não persistem mais no meio do percurso), as relações leem a
chave do pai da linha atual do cliente em vez do cursor defasado do
servidor, e `Seek`/`SetIndexOrder`/`GetRecord` não servem mais linhas
pré-carregadas obsoletas. O read-ahead agora funciona também em
browses **com ordem** (um percurso ordenado de 299 linhas caiu de 598
viagens de rede para 7), `AdsCacheRecords` é respeitado, e
`GotoTop`/`Seek` retornam a linha alcançada, então a primeira leitura
após reposicionar não custa uma viagem extra.

Atualize **ambos** os binários: `openads_serverd` e `openace64.dll` —
várias dessas correções são do lado cliente. Novo exemplo:
`examples/fivewin/xbrowse_delscope.prg` (xBrowse FWH, remoto,
`SET DELETED ON` + scope de índice, apagados dentro do scope).
...
Continuação da v1.8.10: o indicador agora também chega ao servidor
quando `SET DELETED ON` é executado **antes** de `AdsConnect60` — a
ordem usada por qualquer aplicação rddads / FiveWin. O cliente
sincroniza o estado logo após conectar, e o servidor reaplica-o à
conexão ABI (criação preguiçosa) usada para a navegação
ordenada/com âmbito.

Requer `openace64.dll` **e** `openads_serverd` atualizados. Testes:
`remote scoped walk honours SET DELETED ON issued before connect`
(+ `… before first ordered op`); sonda de rede
...
### ABI — padding de chaves `OrdScope` em campos carácter

Harbour envia o âmbito com comprimento aparado, mas as chaves CDX são
preenchidas com espaços. `setScopeTop` / `setScopeBottom` num campo de
ordem de trabalho podia deixar `GotoTop` em EOF em **local e remoto**.
`AdsSetScope` agora preenche até `key_length`. Teste: `QA-D`.

---

## Destaques v1.8.0

### CDX — colação OEM NTXPL852 / PL852

`AdsSetCollation` aceita `NTXPL852` e `PL852` para ordenação de índices
CDX em CP-852 polonês. Ł (0x9D) fica entre L e M em build, seek, insert
e reindex.

### CDX — `REINDEX` em bloco

`Table::reindex()` em tags CDX usa `build_bulk()` (mesmo caminho rápido
que `CREATE INDEX`).

### Testes

19 novos testes unitários (motor, driver CDX e ABI).
...
### Segurança e endurecimento remoto

Path jail no `Connect` remoto, unlock aninhado coerente no `LockMgr`,
verificação TLS por defeito, e escritas remotas de memo/Unicode/data/raw.

### Harbour smoke CI

Job Windows no GitHub Actions + scripts `run_harbour_smoke.ps1` /
`bootstrap_harbour_ci.ps1`.

### ABI remoto (Fase 2)

`AdsSetRelation`, `AdsSetRecord` / `AdsGetRecord`, `AdsCustomizeAOF` em
`tcp://`. `AdsAggregate` / `AdsFetchWhere` local em tabelas DBF.

### Plus — write navegacional SQLite e MSSQL

Backends SQLite e MS SQL Server nativo (TDS) com
`AdsAppendRecord` / `AdsSetString` / `AdsWriteRecord` /
`AdsDeleteRecord` — paridade com MariaDB, PostgreSQL, Firebird e ODBC.
Padding de `ADS_STRING` no read MSSQL.

### Motor

Fixtures ADT/ADI no repositório; cabeçalho VFP `0x32` (autoinc + nullable).
