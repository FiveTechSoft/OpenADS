...
### Added — iMac SetScope fix, delscope test data, AdsSetDouble test
...
## 1.8.21 ÔÇö 2026-07-21

### Added ÔÇö server prints version number when launched
...
The `ads_err.dbf` / error-log ÔÇ£server startedÔÇØ entry also records the
version so Windows Service launches (no console) still leave a clear
trail.

## 1.8.20 ÔÇö 2026-07-21

### Fixed ÔÇö Windows release packages ship Studio HTTP (`OPENADS_WITH_HTTP=ON`)
...
### Fixed ÔÇö Harbour rddads: `ADS_DD_DISABLE_DLL_CACHING` undeclared

`include/openads/ace.h` was missing SAP data-dictionary property IDs
121ÔÇô130. Harbour `contrib/rddads` uses
...
against the updated `ace.h` (or this releaseÔÇÖs headers).

## 1.8.19 ÔÇö 2026-07-20

### Fixed ÔÇö remote xBrowse ghost rows with scope + SET DELETED ON
...
2. Remote: rddads `OrdKeyCount` calls `AdsGetRecordCount(hOrdCurrent)`.
   That path no longer returns the parent tableÔÇÖs physical `RecCount`;
...
## 1.8.18 ÔÇö 2026-07-20

### Added ÔÇö server filesystem API (`oads_*` / `Ads*`)
...
- Low-level I/O: `AdsFOpen`, `AdsFCreate`, `AdsFClose`, `AdsFRead`,
  `AdsFWrite`, `AdsFSeek`
- Wire opcodes `0xE0`ÔÇô`0xFD`; server gated by **`EnableFileFunc=0`**
...
## 1.8.17 ÔÇö 2026-07-20

### Fixed ÔÇö remote DbCreate / AdsCreateTable wrote next to the client app

When a Harbour/X# client connected with `AdsConnect60(tcp://ÔÇª)` and called
...
## 1.8.16 ÔÇö 2026-07-19

### Fixed ÔÇö DbSeek missed on compound, nested-function and probe-sized CDX keys (#131)
...
logical `"1"`/`"0"` ÔåÆ `'T'`/`'F'` seek mapping, and the nested
`Upper(PadR(LTrim(NAME),10))` case ÔÇö all seek to the expected record.

## 1.8.15 ÔÇö 2026-07-19

### Fixed ÔÇö AdsCreateTable wrote the file next to the client app instead of the ADS data directory

When a client (Harbour rddads, X# ADSRDD, ÔÇª) passed `AdsCreateTable` an
...
silently dropped and the new `.dbf`/`.adt` landed next to the application ÔÇö
...
## 1.8.14 ÔÇö 2026-07-17

### Fixed ÔÇö SQL result cursor was left at BOF after execute (wrong data on every query)
...
executing ÔÇö the SAP-supported pattern, used by countless apps ÔÇö got a
...
so callers that already call it are unaffected ÔÇö which is exactly why the whole
existing suite passed without catching this. Regression test added that reads
a result with no prior `GotoTop`.

### Backward (PgUp) read-ahead ÔÇö 299 ÔåÆ 7 round-trips on a reverse scan
...
as a forward scan ÔÇö a 300-row reverse scan over loopback dropped from 299 wire
requests to 7 (the depth ramp: 8+16+32+64├ù4).
...
- The prefetch lag counter is now **signed** (`cursor_lag = client_logical ÔêÆ
  ...
  The forward path is byte-for-byte unchanged (the lag was always ÔëÑ 0 there),
...
- `AdsMgGetInstallInfo` on a **remote** mgmt handle reports the
  **server's** version (one `Hello` round-trip ÔÇö works against every
...
- `examples/fivewin/xbpaint_delscope.prg` (new) logs that server version
  and drives the exact xBrowse repaint shape ÔÇö anchor / page-read /
  `AdsGetRelKeyPos` / `DbGoto` back / advance, plus the arrow-up variant
  and scrollbar thumb drags via `AdsSetRelKeyPos` ÔÇö over a 300-row
...
table but only *erased the map entry* for that shadow handle ÔÇö the local
...
lives ÔÇö an app-level retry loop turns that into seconds of delay or an
...
## 1.8.13 ÔÇö 2026-07-15

### Remote read-ahead on ordered browses (the Mp10 xBrowse case) ÔÇö 598 ÔåÆ 7 round-trips
...
browse that actually matters paying full price ÔÇö Harbour's rddads
...
- **Index-order look-ahead.** `pack_row_trailer` now resolves an ordered
  table to its ABI handle ÔÇö where the order and any scope live ÔÇö so the
...
- **Adaptive depth.** Sequential detection lives on the server, as in OS
  and DB read-ahead generally: depth ramps 8 ÔåÆ 16 ÔåÆ 32 ÔåÆ 64 per
...
- **Byte budget.** The block is capped at 32 KB as well as by row count ÔÇö
...
- `0` or `1` **turns read-ahead off** ÔÇö SAP's documented remedy for a batch
...
because `0` already means "disable" ÔÇö had absent-been-zero, every
...
### Fixed ÔÇö relations followed a STALE parent row (wrong data, shipped)
...
ÔÇö every Mp10 relation browse ÔÇö showed mismatched child data.
...
- **`SeekAck` now carries the row it landed on.** It used to be just
  `[u8 found][u32 recno]` ÔÇö the client knew *where* it was but not *what*
  was there, so the first `AdsGetField` after a seek paid a second
  round-trip (`FetchCurrentRow`). **Seek-then-read ÔÇö the most common thing a
  business app does ÔÇö cost 2 round-trips; it now costs 1.** The relation code
...
- Deliberately **not** `GotoBottom` (a *forward* block past the last record is
  empty ÔÇö that needs backward look-ahead, which is separate work) and
  ...
  "a skip operation after ... any other movement operation" ÔÇö the skip earns
...
### Fixed ÔÇö a seek/scope op left the read-ahead ramp climbing (code review)
...
restarting at the floor ÔÇö defeating the "seek a record, read it, move on"
...
### Fixed ÔÇö `SET DELETED ON` did not hide deleted rows mid-scan (remote)

Flipping `AdsShowDeleted` changes which rows are *visible*, but it did not drop
the client's read-ahead block ÔÇö and that block holds rows already read **ahead**
...
sequential prefetch (M12.21) ÔÇö each is correct alone. `AdsShowDeleted` now
...
### Fixed ÔÇö stale-row bugs in the existing prefetch (wrong data, not just slow)
...
- `AdsCacheRecords` was documented as a no-op because "OpenADS does not
  pre-cache rows" ÔÇö untrue since M12.21. Now describes the automatic
  read-ahead and states plainly that the requested *depth* is still not
  honoured.
- `docs/wire-protocol.md` ┬º5.8 documented a row-trailer layout that has
  never existed in the code. Replaced with the real format, including the
  look-ahead block and the consumed-lag protocol.

### Examples

- `examples/fivewin/xbrowse_delscope.prg` ÔÇö FiveWin xBrowse over a remote
...
## 1.8.12 ÔÇö 2026-07-14

### ENGINE ÔÇö multi-record `Skip` counts *visible* rows under `SET DELETED ON` / filters (M12.33)
...
- The client prefetch fold (M12.21 option C) resynchronises the server
  with `Skip(step + prefetch_consumed)` ÔÇö a multi-record skip. When it
...
walks one record at a time and counts only visible rows ÔÇö same
...
## 1.8.11 ÔÇö 2026-07-14

### REMOTE ÔÇö `SET DELETED ON` issued before connect now reaches the server (M12.32)
...
`AdsConnect60` ÔÇö the startup order virtually every rddads / FiveWin
...
- Server: the `ShowDeleted` handler applied the flag to `abi_conn_`
  ...
  `show_deleted=true` ÔÇö and the per-connection flag overrides the
...
connect` and `ÔÇª before first ordered op` in
...
## 1.8.10 ÔÇö 2026-07-14

### REMOTE ÔÇö `SET DELETED ON` honoured on scoped index walks (M12.31)
...
## 1.8.9 ÔÇö 2026-07-10

### Zero-config OEM collation for rddads apps (`usCharType` honoured, adslocal.cfg) ÔÇö #130 follow-up

The second mechanism from the v1.8.6/v1.8.7 regression report: a
Harbour rddads app has no way to call `AdsSetCollation` ÔÇö its only OEM
signal is `usCharType = ADS_OEM` on `AdsOpenTable` (what
`AdsSetCharType(ADS_OEM)` produces) ÔÇö and OpenADS ignored that
parameter. `INDEX ON` therefore built keys with UTF-8 casing and binary
sort while the app seeks with Harbour's CP-852 `Upper()` keys ÔåÆ
...
- Unsupported `OEM_CHAR_SET` values (USA, MAZOVIA, ÔÇª) leave raw byte
...
## 1.8.8 ÔÇö 2026-07-10

### Fix: NTXPL852 / OEM collation seek broken on reopened CDX bags (#130 regression)

v1.8.6 regression, reported with a clean 1.8.4ÔåÆ1.8.6 bisect: `DbSeek` on
a character tag under NTXPL852 returned not-found for existing records ÔÇö
...
weight order differs from byte order (e.g. `┼ü` = 0x9D).

The reopen path now decides the encoding from the key expression ÔÇö bare
...
- **`ADS_TOP` / `ADS_BOTTOM` constants** (`ace.h`): OpenADS used 0/1; the
  ...
  never set the TOP bound for rddads callers ÔÇö both bounds landed on
...
- **`AdsGetKeyCount` ignored the active scope** on CDX orders ÔÇö it
...
and `DbSeek(date)` as julian-day doubles ÔÇö `AdsSetScope` and `AdsSeek`
...
## 1.8.4 ÔÇö 2026-07-09

### CDX ÔÇö further `INDEX ON` speedups (#128)
...
## 1.8.3 ÔÇö 2026-07-09

### CDX ÔÇö `INDEX ON` / REINDEX performance for local rddads (#128)
...
This directly addresses the ~11├ù slowdown vs SAP ACE 11.10 on real
production tables (e.g. `ASORTYM.DBF` 14k rows / 125 cols / 16 tags) and
full reindex workloads.

Reported-by: rkedzioralmaalpinex

## 1.8.2 ÔÇö 2026-07-08

### CI / release ÔÇö NTXPL852 test fixture (Clang + Linux/macOS builds)

The v1.8.0 NTXPL852 unit tests used adjacent string literals and greedy
`\x` escapes for the Polish ┼ü (OEM `0x9D`) rows. Clang on Linux/macOS
...
No functional engine changes vs v1.8.1 ÔÇö use v1.8.2 release archives for
Linux/macOS binaries.

## 1.8.1 ÔÇö 2026-07-08

### ABI ÔÇö `OrdScope` / `AdsSetScope` string key padding
...
matching row ÔÇö `GotoTop` landed at EOF on **both local and remote**.
...
## 1.8.0 ÔÇö 2026-07-08

### CDX ÔÇö NTXPL852 / PL852 OEM collation (#127)
...
┼ü (0x9D) sorts between L and M, matching Harbour / Clipper / SAP ACE
...
## 1.7.0 ÔÇö 2026-07-08

### REMOTE ÔÇö AdsSetScope / OrdScope navigation fix
...
was set ÔÇö which happened on `SetOrder` but never on `SetScope`. Harbour
...
## 1.6.5 ÔÇö 2026-07-07

### REMOTE ÔÇö OrdKeyCount() fix + AdsGetDate() crash fix
...
## 1.6.4 ÔÇö 2026-07-05

### REMOTE ÔÇö FiveWin TDataBase / ADSRDD integration (critical fixes)
...
## 1.6.3 ÔÇö 2026-07-04

### REMOTE / FWH ÔÇö xBrowse index navigation over `tcp://`
...
- **Bug fix: `AdsGetKeyNum` returned physical `RecNo` instead of logical
  key position** ÔÇö FWH `xBrowse:SetRDD()` uses `AdsKeyNo(, , 1)` for the
...
- **Bug fix: `GotoRecord` left the server ABI index cursor stale** ÔÇö
...
- **Bug fix: `AdsAtBOF` always answered ÔÇ£not BOFÔÇØ while `row_valid`** ÔÇö
...
- **Client: `remote_index_skip(0)`** ÔÇö `Skip(0)` settles prefetch lag
  without clearing `prefetch_consumed` first; index-nav preamble no longer
  invalidates `row_valid` before every skip ack.

- **Regression tests** ÔÇö `remote bookmark restore keeps AdsGetKeyNum
...
## 1.6.2 ÔÇö 2026-07-04

### REMOTE / FWH ÔÇö production CDX tag names & append-row FieldGet

- **Bug fix: SAP production CDX tag names corrupted** ÔÇö `CdxIndex::list_tags()`
...
- **Bug fix: `AdsGetField` at BOF/EOF returned 5000** ÔÇö FWH
...
- **Regression tests** ÔÇö `abi_no_current_record_test`,
...
## 1.6.1 ÔÇö 2026-07-02

### REMOTE mode ÔÇö production CDX auto-open in subdirectories

- **Bug fix: `ensure_abi_handle()` used basename-only table paths** ÔÇö When
...
- **Regression tests** ÔÇö `remote production CDX auto-open in subdirectory`
...
## 1.6.0 ÔÇö 2026-07-01

### REMOTE mode ÔÇö FWH production CDX validation & bug fixes

- **Bug fix: `AdsGetNumIndexes` in REMOTE mode** ÔÇö Previously returned 0
...
- **Bug fix: implicit GoTop after `AdsOpenTable90` in REMOTE mode** ÔÇö
...
- **New test: `abi_remote_prodcdx_test.cpp`** ÔÇö 8 test cases that
...
- **FieldGet by ordinal idiom documented** ÔÇö ACE's `ADSFIELD(n)` casts
...
- **Gated on `OPENADS_TEST_REMOTE`** ÔÇö set to a server URI (e.g.
  `tcp://192.168.18.184:16262/`) to run against a live remote server.
  Skipped in default CI.

- **DOING.md** added ÔÇö live working-notes file tracking in-progress
  investigation, test results, and pending fixes.

## 1.5.2 ÔÇö 2026-06-27

### Release packaging

- **Windows x86 (32-bit) archive restored** ÔÇö v1.5.1 shipped only
...
- **Harbour smoke** ÔÇö green on GitHub Actions (`contrib/rddads` bootstrap,
  fresh `openace64.lib` link).

## 1.5.1 ÔÇö 2026-06-27

### Security & remote hardening

- **Path jail on remote Connect** ÔÇö client paths are canonicalized and
  confined under `openads_serverd --data`; traversal attempts are rejected.
- **LockMgr nested unlock** ÔÇö OS byte locks remain held until the final
  nested `unlock_*` releases them.
- **Remote field writes** ÔÇö `AdsGetMemoDataType`, `AdsSetStringW`,
  `AdsSetJulian`, and `AdsSetFieldRaw` route through `tcp://`.
- **TLS** ÔÇö peer certificate verification on by default;
...
- **`tools/scripts/run_harbour_smoke.ps1`** and
  **`bootstrap_harbour_ci.ps1`** ÔÇö portable Harbour bootstrap for CI.

### Remote ABI ÔÇö Fase 2 closed

- **`AdsSetRelation` / `AdsSetScopedRelation`** ÔÇö parentÔåÆchild relations on
  local and `tcp://` tables; `apply_relations_for_handle()` after navigation.
- **`AdsSetRecord` / `AdsGetRecord`** ÔÇö wire opcodes `0xA8`ÔÇô`0xAB`.
- **`AdsCustomizeAOF`** ÔÇö wire opcodes `0xAC`/`0xAD`.
- **`AdsAggregate` / `AdsFetchWhere`** ÔÇö local in-process DBF tables (SQL
  backends via existing aggregate path).

### SQL backends ÔÇö navigational write (Plus)

- **SQLite write** ÔÇö `AdsAppendRecord`, `AdsSetString`, `AdsWriteRecord`,
  `AdsDeleteRecord` with rowid-keyed DML and parameterized binds
  (`abi_plus_sqlite_write_test.cpp`, in-process).
- **MSSQL native (TDS) write** ÔÇö same ABI surface; PK discovery via
...
- **MSSQL read fix** ÔÇö `ADS_STRING` fields padded to declared width in
...
- **VFP header `0x32`** ÔÇö autoinc and nullable columns together;
  `_NullFlags` synthetic column when the NULL bitmap is present.

### SQL Tier-1 wiring

- **`BackendTxManager` hooks** ÔÇö `AdsBeginTransaction` / commit / rollback /
  `AdsSetAutoCommit` on SQLite and PostgreSQL; DML auto-commit after write.
- **Tier-1 utilities in execution** ÔÇö field optimizer and where builder
  drive actual SQL generation on SQLite reads.

### Fixes

- **AOF V2** ÔÇö `Like` / `IsNull` ops handled in `aof_eval` (clang `-Wswitch`).
- **Harbour CI bootstrap** ÔÇö track `harbour/core` master.
- **Concurrent SQLite test** ÔÇö tolerate minor `SQLITE_BUSY` under contention.

## 1.5.0 ÔÇö 2026-06-27
...
- **`$` contains: field-to-field support.** `field1 $ field2` now
  ...
  semantic mismatch.
- **`SqlDialect` expansion.** New fields: `length_fn` (LEN ÔåÆ
  LENGTH/CHAR_LENGTH), `now_fn` (DATE() ÔåÆ NOW()/CURRENT_DATE),
...
- **Bulk-load index builder (`build_bulk`).** New bottom-up
  B+tree construction path for `CREATE INDEX` ÔÇö approximately 10├ù
...
- **O(1) browse position cache.** `ordered_recnos_cached()` and
  `pos_of_recno_cached()` on `CdxIndex` cache the keyÔåörecno
  mapping so `AdsGetRelKeyPos` / `AdsGetKeyNum` answer from an
  in-memory vector instead of walking the index.
- **CDX conditional (FOR) index predicates ÔÇö persist + apply.**
  `CREATE INDEX ... FOR <condition>` now persists the condition in
  the CDX sub-tag header and applies it at insert time ÔÇö only
...
- **Composite CDX key width not pinned to the 254-byte probe.**
  ...
  evaluation probe ÔÇö it uses the actual key width, so composite tags
...
- **SQL concurrency safety ÔÇö stmt_map serialisation.** Concurrent
...
- **CDX index direction fix for Harbour rddads (FiveWin).** `AdsCreateIndex61`
  ...
  **swapped** positions: X#'s ADSRDD sends `0x02` for an ascending tag and
  ...
  reversed ÔÇö `AdsGotoTop` landed on the last key and `Skip` walked backward,
...
- **CDX character index key width fix (PR #68).** `AdsCreateIndex61`
  ...
  record ÔÇö both inside the index and for native FoxPro/Clipper readers
...
- **CDX leaf recno bits + prefix seek (PR #62).** Two correctness
  ...
  (ÔëÑ40 bytes) no longer silently truncate recnos ÔëÑ 4096; (2)
...
## 1.2.0 ÔÇö 2026-06-24

- **Deferred-flush bulk-insert mode (528├ù speedup).** A new
  ...
  (50 rec/s). Remote benchmark (Windows client ÔåÆ iMac server over
  WiFi tcp://): 500K records in 0.69s at 784K rec/s ÔÇö 36├ù faster
  than local mode. 649/649 unit tests pass; backward-compatible ÔÇö
...
- **SQL backends: PostgreSQL / MariaDB / ODBC behind a pluggable
  ...
  per-backend `if` block ÔÇö adding a further backend is one ops
...
- **CDX stale record-count refresh on the fetch path (PR #50).** A
  ...
  just-appended recno (e.g. mid-`REPLACE ÔÇª FOR` / DBEVAL) then
  ...
  unlocked-refresh fallback. Slow path only ÔÇö a normal forward scan
  never reads past the count, so the single-writer case pays
  nothing.

## 1.0.3 ÔÇö 2026-06-23

- **Round-trip-thrifty remote scan (PR #47).** A forward scan over
  the `tcp://` wire no longer costs ~one TCP round-trip per record.
  A sequential-prefetch path ÔÇö negotiated via a Connect capability
  flag ÔÇö piggybacks a lookahead block onto forward-`Skip` acks; the
  ...
  from a cached `Found()` flag. A 50k-record loopback scan is ~3.9├ù
  faster (NAV-only) / ~3.3├ù (3-field read), `IsFound` round-trips
...
- **Cookbook (PR #44).** New `cookbook/` folder with runnable,
  heavily-commented Harbour examples ÔÇö a `console/` track (pure
...
## 1.0.0-rc29 ÔÇö 2026-05-26

- **Turnkey `hbmk2` (`.hbp`) example for Harbour apps ÔÇö
  `examples/harbour-hbmk2/`.** Reported on the FiveTech forum:
  *"alguna alma caritativa que proporcione un archivo de
  compilaci├│n `.hbp` para crear un programa con OpenADS ÔÇö todos
...
- **`AdsGetField` pads CHARACTER fields to the declared width.**
  ...
  field's declared width on the way out ÔÇö both the local and the
...
- **PHP binding ÔÇö result-fetch and index seek.** `bindings/php`
...
- **CI** ÔÇö the release workflow gains a macOS Intel (x64) build
  leg, so releases ship a `macos-x64` archive alongside
  `macos-arm64`.

## 1.0.0-rc26 ÔÇö 2026-05-16

- **PHP binding ÔÇö `bindings/php`.** Reinaldo Crespo asked whether a
  ...
  - **`openads/openads-php`** ÔÇö a pure-PHP Composer package, no
  ...
  - **Parameterised SQL.** `Statement::query()` accepts `?`
    ...
    statement) ÔÇö the anti-injection boundary, with its own unit
    tests.
...
- **SQL `''` string-escape fix.** `read_string_literal` in the SQL
  ...
  string `O` followed by a stray token ÔÇö error 7200. Any SQL
...
- **Index correctness sweep.** Three bugs that broke CDX/NTX
  ...
  - **`AdsCreateIndex61` decoded the wrong option bit.** `ace.h`
    ...
    `ulOptions & 0x08` ÔÇö that bit is `ADS_COMPOUND`, which both
  ...
  - **`ALIAS->FIELD` qualifiers in index expressions.** Harbour
    ...
    to the RDD. `evaluate_index_expr` could not parse it ÔÇö the
...
- **`AdsMg*` server-telemetry subsystem.** Reported by Pritpal Bedi
  ...
  - **`MgCollector`** (`src/mgmt/`) ÔÇö single source of truth. Formats
  ...
  - **Honesty.** Fields with no OpenADS analogue ÔÇö checksum failures,
    NetWare-era ECB counts, per-category memory, serial number ÔÇö
...
- **Harbour `contrib/rddads` clean-compile sweep.** Reported by
  ...
  `HB_WITH_ADS=ÔÇª/openads/include/openads hbmk2
  ...
  - **`AdsSetScope`** ÔÇö 3-arg `(hIndex, usScope, pucKey)` ÔåÆ
    ...
    `AdsSeek`'s `ADS_DOUBLEKEY ÔåÆ ASCII-padded numeric`
    conversion so a scope set with a `double` compares
    apples-to-apples against the index's stored key bytes.
  - **`AdsGetVersion`** ÔÇö 4-arg with mistyped letter/desc slots
    (`UNSIGNED32*` x4) ÔåÆ 5-arg
    ...
    stack.
  - **`AdsCopyTableContents`** ÔÇö 2-arg ÔåÆ 3-arg with
    `usFilterOption`. Filter mode currently accepted and
    documented; `IGNOREFILTERS` is the implemented path.
  - **`AdsCreateSavepoint` / `AdsRollbackTransaction80`** ÔÇö 2-arg
    ÔåÆ 3-arg with reserved `ulOptions` parameter (matches ACE 8.x).
  - **`AdsGetAOF`** ÔÇö `(ADSHANDLE, UNSIGNED32*, UNSIGNED32*)`
    (returned-record-count style) ÔåÆ `(ADSHANDLE,
    UNSIGNED8* pucFilter, UNSIGNED16* pusLen)`. Returns empty
    filter for now; full AOF-source-string round-trip lands with
    M-AOF.4.
  - **`AdsEvalAOF`** ÔÇö 3rd arg was `UNSIGNED32* pulRecords`; SAP
    expects `UNSIGNED16* pusOptLevel` (returns `ADS_OPTIMIZED_*`).
  - **`AdsSetStringW` / `AdsGetStringW` / `AdsGetFieldW`** ÔÇö field
    ...
    to match; UTF-16 ÔåÆ UTF-8 transcode dropped from the name path
    (it was only there to compensate for the wrong type).
  - **`AdsGetString` / `AdsGetLong`** ÔÇö exports were already
    ...
    added.
  - **`ADS_MAX_PARAMDEF_LEN`** ÔÇö `#define` is now `#ifndef`-guarded
    so a Harbour-style pre-define (`#define ADS_MAX_PARAMDEF_LEN
    2048` before the include) is honoured silently.
  - **`AdsGotoBookmark60`** ÔÇö was 2-arg `(hObj, *pucBookmark)`;
    ...
    updated.
  - **`AdsGetAllTables`** ÔÇö was 2-arg `(*ahTable, *pusArrayLen)`;
...
## 1.0.0-rc22 ÔÇö 2026-05-13

- **M12.25 ÔÇö `AdsCreateTable` stamps the DBF header last-update date.**
  ...
  into header bytes 1..3 up front, so OpenADS now does too ÔÇö in
...
## 1.0.0-rc21 ÔÇö 2026-05-12

- **M12.24 ÔÇö `AdsGetLastTableUpdate` real signature + AOF
  ...
  last-updated stamp (header bytes 1..3, year offset from 1900) ÔÇö over
  the wire too via a new `GetLastTableUpdate` opcode ÔÇö and renders it
...
- **`AdsSetAOF` no longer fails error 7200** on a non-optimisable
  expression (e.g. `Empty(NAME)`, `UPPER(NAME) = 'A'`) ÔÇö ACE treats
...
## 1.0.0-rc20 ÔÇö 2026-05-12

- **`OPENADS_WITH_HTTP` defaults to ON.** Studio no longer needs the
  explicit CMake flag ÔÇö pass `-DOPENADS_WITH_HTTP=OFF` to opt out.
- **`AdsGetKeyNum` returns the correct relative key position.**
- **FiveWin + xBrowse over ADS** sample under `examples/fivewin/`.

## 1.0.0-rc19 ÔÇö 2026-05-12 ÔÇö X# Advantage RDD compatibility (local + remote)

- **M12.22 ÔÇö versioned ACE overloads for the X# RDD.** Exports the
...
- **M12.23 ÔÇö close the export gap the X# Advantage RDD relies on.**
  ...
  enforcement toggles, `AdsStmt*` helpers, ÔÇª). Forwards where one
...
- **`AdsAppendRecord` auto-locks the new record** (ACE semantics for
  non-exclusive tables ÔÇö X#'s `GoHot` refuses to write a record it
  sees as unlocked).
...
- **`AdsCreateIndex61` / `AdsCreateIndex90` option-bit fix:** the
  "descending" flag is `ADS_DESCENDING` (`0x08`), not `0x02` ÔÇö `0x02`
...
- **`AdsCreateTable` / `AdsCreateTable90` stage an empty `.fpt` next
  ...
  `usMemoBlockSize`, default 64) ÔÇö without it `Connection::open_table`
...
- **X# RDD against a remote OpenADS server.** Three remote-path
  ...
  (`AdsConnect60("tcp://host:port/<datadir>", ADS_REMOTE_SERVER) ÔåÆ
  AX_SetConnectionHandle ÔåÆ DbUseArea`): `remote_field_index` now
  honours the "field name OR 1-based ordinal cast to a pointer" idiom
  (X#'s `_FieldSub` calls `AdsGetFieldType` / `Length` / `Decimals`
  by ordinal ÔÇö a tiny pointer value was being dereferenced as a
...
- **Test layout.** Third-party RDD smoke harnesses under
  ...
  WinForms) under `examples/`. All opt-in ÔÇö none run in default
...
- **CDX FOR-clause filter.** `CREATE INDEX ÔÇª FOR <expr>` honoured at
...
## 1.0.0-rc18 ÔÇö 2026-05-09 ÔÇö wire-protocol perf overhaul (~30├ù xbrowse repaint)

- **M12.17 ÔÇö `RemoteTable` row cache.** New `FetchCurrentRow` opcode
  ...
  RTT.
- **M12.18 ÔÇö piggyback row trailer on nav acks.** `GotoTopAck` /
  ...
  all hit the cache populated by the prior nav ack ÔÇö zero extra RTT
  after every nav.
- **M12.19 ÔÇö record_count cache.** `AdsGetRecordCount` and
  ...
  `AdsZapTable`).
- **M12.20 ÔÇö `TCP_NODELAY`** disabled Nagle on every per-connection
  socket. The OpenADS wire is strict ping-pong, so Nagle's
  accumulation delay (up to 200 ms) was pure latency tax. Removes
  ~40ÔÇô200 ms per RTT on slow links.
- **Net xbrowse PgDn**: pre-M12.17 ~300 RTT ÔåÆ ~20 RTT ├ù ~5 ms (Nagle
  off) Ôëê ~100 ms. ~30├ù end-to-end speedup vs pre-M12.17.

## 1.0.0-rc17 ÔÇö 2026-05-09 ÔÇö full wire bridges (rddads parity) + AOF/Rushmore + Studio Demo
...
- **M12.14 ÔÇö field metadata + cursor state.** `AdsGetNumFields`,
...
- **M12.14b ÔÇö typed reads.** `AdsGetLong`, `AdsGetDouble`,
  `AdsGetJulian` reuse `GetField`, parse client-side. No new opcode.
- **M12.15 ÔÇö info / lock / maintenance / AOF.** `AdsIsFound`,
...
- **M12.15b ÔÇö memo.** `AdsGetMemoLength`, `AdsBinaryToFile`,
  `AdsFileToBinary` reuse `GetField` / `SetField`.
- **M12.16 ÔÇö index handle subsystem.** New
...
## 1.0.0-rc14 ÔÇö 2026-05-08 ÔÇö Windows Service + systemd / launchd units
...
- **Linux systemd unit** (`scripts/openads-serverd.service`)
  hardened ÔÇö `User=openads`, `ProtectSystem=strict`,
  `NoNewPrivileges`, `RestrictAddressFamilies=AF_INET AF_INET6`,
  `PrivateTmp`, `Restart=on-failure`.
- **macOS launchd plist**
  (`scripts/com.openads.serverd.plist`) ÔÇö KeepAlive on crash;
  stdout / stderr to `/var/log/openads-serverd.{out,err}.log`.

## 1.0.0-rc13 ÔÇö 2026-05-08 ÔÇö production-CDX auto-open + Studio Demo + version fix

- **M-AOF.6 ÔÇö production-CDX auto-open in `AdsOpenTable`.** Opening
  `<base>.dbf` auto-binds the sibling `<base>.cdx` so every tag in
  it becomes navigable on the Table without an explicit
  `AdsOpenIndex60` call. rc12 didn't do that ÔÇö Studio's per-request
...
## 1.0.0-rc12 ÔÇö 2026-05-08 ÔÇö AOF (Rushmore-style) full slice

First working slice of **Advantage Optimised Filters (AOF) ÔÇö
...
## 1.0.0-rc10 ÔÇö 2026-05-08

- **Studio mode badge.** SPA header now shows ­ƒÅá `LocalServer`
  (green) when the console runs in-process inside `ace64.dll` /
  `ace32.dll`, or ­ƒîÉ `Remote Server` (blue) when hosted by
  ...
  deployments that strip unknown fields keep working unchanged ÔÇö
  the badge stays hidden when `/api/health` is unreachable.

## 1.0.0-rc9 ÔÇö 2026-05-08 ÔÇö embedded Studio (LocalServer)

Studio web console is now embedded inside `ace64.dll` / `ace32.dll`
itself. A LocalServer application ÔÇö Harbour / X# / Clipper loading
the OpenADS DLL directly without launching `openads_serverd.exe` ÔÇö
spins up the console in its own process.

- Three OpenADS-only entry points:
  `AdsStudioStart(port, data_dir)`, `AdsStudioStop()`,
  `AdsStudioPort(&port)` (ordinals 238ÔÇô240).
- Env-driven auto-start: `OPENADS_STUDIO_PORT=<port>` before
  ...
  hook is a no-op ÔÇö no surprise localhost listener.
...
## 1.0.0-rc8 ÔÇö 2026-05-08 ÔÇö dual x64+x86, static mbedtls
...
## 1.0.0-rc4 ÔÇö 2026-05-07

CDX leaf+branch full multi-level split + memo > 64 K fix +
cross-platform `-Werror` clean.

- **`M(cdx-split)`** ÔÇö full multi-level B+tree leaf+branch split.
  Stress harness now exercises 2 bags ├ù 2 tags each at 200 K rows;
...
- **TLS deployment guide** (EN / ES / PT) ÔÇö proxy, stunnel, SSH
  tunnel recipes for fronting the cleartext server with TLS until
  M12.12 ships.
- CI release-workflow build timeout bumped 45 ÔåÆ 60 minutes.

## 1.0.0-rc2 ÔÇö 2026-05-07

Studio web 0.4 ÔÇô 0.10: Sessions, Data Dictionary tab + REST,
...
## 1.0.0-rc1 ÔÇö 2026-05-07

Studio web console + bilingual+ documentation site + release CI.

- **OpenADS Studio** (`studio.web.0.1` ÔÇª `0.3`) ÔÇö clean-room
...
## 0.3.6 ÔÇö 2026-05-06

`tls://` URI scheme reserved (M12.12 stub) ahead of the real
handshake landing in 0.4.0.

## 0.3.5 ÔÇö 2026-05-06

Wire-protocol semantics complete: ACE error-code propagation
(M12.10) and batched row fetch (M12.11 ÔÇö `Fetch` / `FetchAck`).

## 0.3.4 ÔÇö 2026-05-06

Phase 2 server feature-complete (read + write + SQL + index +
auth).

- **M12.6** ÔÇö remote write surface (`append` / `set` / `delete` /
  `recall` / `goto` / `flush`).
- **M12.7** ÔÇö remote SQL exec (`ExecuteSQL` wire op).
- **M12.8** ÔÇö remote `AdsReindex`.
- **M12.9** ÔÇö server-side authentication.
- `RemoteConnection` dtor now calls `disconnect`.

## 0.3.3 ÔÇö 2026-05-06

Phase 2 server alive end-to-end + cross-platform SQL bench.

- **M12.3** ÔÇö server accept loop + `Hello` / `Connect` dispatch.
- **M12.4** ÔÇö remote read-only table ops.
- **M12.5** ÔÇö dual-mode DLL: `tcp://` URIs route the ABI to the
...
- **M10.54** `FILTER (WHERE ÔÇª)` aggregate clause.
- **M12.2** sockets layer (cross-platform).

## 0.3.1 ÔÇö 2026-05-05
...
- **JOIN matrix.** `INNER` (M10.13 parse + M10.14 executor), `LEFT
  OUTER` (M10.16), `RIGHT OUTER` (M10.21), `FULL OUTER` (M10.22 ÔÇö
...
- **DML / DDL.** `INSERT` (M10.5), `ORDER BY` execution (M10.6),
  ...
  (M10.12), `INSERT INTO ÔÇª SELECT` (M10.41), `CREATE TABLE AS
  SELECT` (M10.42).
- **Other SQL.** `WHERE` `OR` / `NOT` / parens + numeric literals
  (M10.3), scalar string fns (M10.39), arithmetic in projection
  (M10.40).
- **Engine + drivers.** Real `.add` Data Dictionary persistence ÔÇö
...
- **Encrypted DBFs (M11.2).** OpenADS-encrypted DBF ÔÇö AES-256-CTR
...
## 0.2.0 ÔÇö 2026-05-04

The 0.2.0 release closes the entire 226-symbol Harbour-reachable
`Ads*` ABI surface ÔÇö every export resolves to either a real
...
- **Compound CDX expression evaluator.** `UPPER`, `LOWER`,
  ...
  case map, `├┐Ôåö┼©` pair); the Latin-1 case mapping table closes the
  M9.17 `*W` Unicode surface.
- **Real CRUD for tables and indexes.** `AdsCreateTable` parses the
  rddads `NAME,Type,Len,Dec;ÔÇª` field-def syntax; `AdsCreateIndex61` /
...
- License: relicensed **MIT ÔåÆ Apache License 2.0** (`LICENSE` +
  `NOTICE`).
- Independence + non-commercial purpose + clean-room provenance +
  no-warranty + downstream responsibility ÔÇö block added to the
...
### M8.9 ÔÇö Multi-tag CDX + OrdSetFocus
...
### M8.10 ÔÇö Transactions through Harbour
...
- A real Harbour app drives `AdsBeginTransaction` /
  ...
  index entries persist by design ÔÇö `Found()` still reports `T` but
...
- `Table::snapshot_index_keys_()` captures the pre-write key per
  index ÔÇö active order plus extras ÔÇö and `sync_all_indexes_(snap)`
...
DBF, walks records, runs `dbSeek`, appends rows, and reopens ÔÇö every
call lands on OpenADS' `ace64.dll` with no Harbour rebuild.

### M0ÔÇôM3.10 (engine + drivers)

- 5-layer architecture: ABI shim ÔåÆ Session/Connection ÔåÆ SQL ÔåÆ Engine
  (Table / Index / MemoStore / LockMgr / TxLog) ÔåÆ OS abstraction.
...
- Group commit (M5.4): each record carries a monotonic LSN;
  `sync_to(lsn)` is the group-commit primitive ÔÇö first thread to
...
### M8 ÔÇö Harbour conformance (this release)

- **M8.0** `ace64.dll` / `ace32.dll` SHARED CMake target with a `.def`
  exporting 80 real `Ads*` entry points.
- **M8.1** 226 `Ads*` exports ÔÇö superset of every symbol Harbour
...
- **M8.4** ACE field-type constants verified by sweeping
  ...
  `ADS_STRING = 4`, ... ÔÇö the inverse of the public ACE SDK ordering
...
- **M8.6** `dbSeek` end-to-end through OpenADS' CDX. `Table::path()`,
  ...
  (accepts table or index handles ÔÇö rddads' `adsGoTop` calls
...
- One `tests/harbour_smoke/` integration harness producing a
  runnable `smoke.exe` that exercises the full Harbour ÔåÆ
  rddads.lib ÔåÆ OpenADS path.
## 1.8.22 ÔÇö 2026-07-22

### Added ÔÇö TCP connection authentication
...
## 1.8.23 ÔÇö 2026-07-22

### Fixed ÔÇö Harbour patch, filesystem exports, Studio folders and CDX paths
